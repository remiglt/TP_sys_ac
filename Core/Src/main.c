...
int main(void)
{
	...
	if(HAL_OK != HAL_ADC_Start_DMA(&hadc1, ADC1_buffer, ADC_BUF_SIZE))
		Error_Handler();
	...
	while (1)
	{
		if (flagADC1==1)
		{
			...
			for(idxADC1=0;idxADC1< ADC_BUF_SIZE; idxADC1++)
			{
				...
				ADC1ValueMoy += ADC1_buffer[idxADC1];
			}
		...
		if(newCmdReady){
			...
			{
				alphaValue1 = (atoi(argv[1])*TIM1->ARR)/100;
...
void CurrentCalcul()
{
	currentADC = (ADC1ValueMoy/4095)*3.3;
	currentADC = (currentADC-2.5)*12;
}
